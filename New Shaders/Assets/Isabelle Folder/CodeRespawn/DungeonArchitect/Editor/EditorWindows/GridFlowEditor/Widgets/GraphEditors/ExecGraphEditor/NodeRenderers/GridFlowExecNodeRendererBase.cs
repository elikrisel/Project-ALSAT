...
using DungeonArchitect.Builders.GridFlow.Graphs;
...
{
    public abstract class GridFlowExecNodeRendererBase : GraphNodeRenderer
    {
        ...
        protected GridFlowExecNodeHandler GetHandler(GraphNode node)
        {
            var execNode = node as GridFlowExecRuleGraphNode;
            return (execNode != null) ? execNode.nodeHandler : null;
        }
        ...
        void DrawNodeExecStatus(UIRenderer renderer, GraphNode node, GraphCamera camera, Rect boxBounds, ExecRuleNodeLayoutInfo layout)
        {
            ...
            if (execState != null && execState.ExecutionStage != GridFlowGraphNodeExecutionStage.NotExecuted)
            {
                ...
                {
                    if (execState.Success == GridFlowExecNodeHandlerResultType.Success)
                    {
            ...
            // Calculate the error size
            {
                ...
                if (ruleNode != null && ruleNode.executionStatus != null)
                {
                    layout.drawErrorMessage =
                        (ruleNode.executionStatus.Success != GridFlowExecNodeHandlerResultType.Success)
        ...
        void DrawNodeBody(UIRenderer renderer, GraphRendererContext rendererContext, GraphNode node, GraphCamera camera)
        {
            ...
            var outputPin = node.OutputPin as GridFlowExecGraphNodePin;
