...
namespace DungeonArchitect.Builders.GridFlow.Graphs.Exec.NodeHandlers
{
    ...
    public class GridFlowExecNodeHandler_FinalizeGraph : GridFlowExecNodeHandler
    {
        ...
        GridFlowAbstractNodeRoomType CalculateRoomType(GridFlowAbstractGraph graph, GridFlowAbstractGraphLink[] incoming,
                GridFlowAbstractGraphLink[] outgoing, int numEnemies)
        {
            if (incoming.Length == 1 && outgoing.Length == 1 && numEnemies == 0) 
            {
                ...
                if (sameLine)
                {
                    return GridFlowAbstractNodeRoomType.Corridor;
                }
            }

            return numEnemies <= maxEnemiesPerCaveNode
                ? GridFlowAbstractNodeRoomType.Cave
        ...
        GridFlowAbstractNodeRoomType GetNodeRoomType(GridFlowAbstractGraph graph, GridFlowAbstractGraphNode node)
        {
            ...
            if (roomType == GridFlowAbstractNodeRoomType.Corridor && !generateCorridors)
            {
                roomType = GridFlowAbstractNodeRoomType.Cave;
            }
            if (roomType == GridFlowAbstractNodeRoomType.Cave && !generateCaves)
            {
                roomType = GridFlowAbstractNodeRoomType.Room;
            }
